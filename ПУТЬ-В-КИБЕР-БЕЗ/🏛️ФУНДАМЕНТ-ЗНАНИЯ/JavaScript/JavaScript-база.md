...
let b = a;
b = 'alert(1)';
console.log(a); // 'XSS' - не изменилось, потому что примитив
...
// userInput.script ТОЖЕ ИЗМЕНИЛСЯ - бага для XSS

// 2. Мутация прототипов (Prototype Pollution)
let obj = {};
obj.__proto__.polluted = 'XSS payload';
// Теперь у ВСЕХ объектов есть свойство polluted!
let newObj = {};
console.log(newObj.polluted); // 'XSS payload' - пиздец
...
let result = 'User input: ' + maliciousObj; // ТРИГГЕР XSS!
```
...
function isNumberTooLong(num) {
    return num > 1000; // Если num - BigInt, сравнение работает!
}
...
try {
    ...
    console.log(e.stack); // Иногда можно получить пути/имена
}
...
console.log(typeof []);           // "object" - ВОТ ЭТО ПОДВОД!
...
// 1. Маскировка кода под "безопасные" типы
function isDangerous(input) {
    return typeof input === 'function'; // Функции считаем опасными
}

let payload = '<script>alert(1)</script>';
// Обходим: делаем payload строкой, а не функцией
if (!isDangerous(payload)) {
    eval(payload); // ОЙ! Строка тоже может быть опасной!
}

// 2. Использование бага с null
function processInput(input) {
    if (typeof input === 'object' && input !== null) {
        return input.toString(); // Думаем, что это объект
    }
    return 'safe';
}
...
// XSS-вектор: обход фильтров через числа
function filterXSS(input) {
    return input.replace(/[<>]/g, ''); // Удаляет < и >
}
let attack = ['<', 'script', '>']; // Массив
let filtered = filterXSS(attack); 
// attack.toString() = "<,script,>" - запятые остались, но фильтр сработал?
// Результат: "<,script,>" - угловые скобки удалились? Нет, они были в строке массива!
...
```javaScript
// ЯВНОЕ ПРИВЕДЕНИЕ (программист сам вызывает функции)
...
// ToString, ToNumber, ToBoolean - алгоритмы
...
String(obj); // "42" (использует toString после valueOf?)

// На самом деле порядок зависит от контекста
