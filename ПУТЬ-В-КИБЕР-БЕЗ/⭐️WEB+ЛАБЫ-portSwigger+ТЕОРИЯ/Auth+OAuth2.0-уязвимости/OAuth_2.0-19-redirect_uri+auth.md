...
## шаг четвертый
формирую ссылку для запроса выше
```http
...
**"Недостаточная валидация redirect_uri"** 
**"Открытое перенаправление в OAuth"**

cсервер может иметь свой редирект и формировать запрос самостоятельно, это позволит использовать только свои адресса.

то есть использовать белые списки адресов на которые сервер может отвечать!