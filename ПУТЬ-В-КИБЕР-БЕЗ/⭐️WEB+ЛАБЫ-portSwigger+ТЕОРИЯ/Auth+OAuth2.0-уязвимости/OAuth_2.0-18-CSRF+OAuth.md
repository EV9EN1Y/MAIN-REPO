...
# CSRF + OAuth
🔶🔶🔶🔶🔶🔶🔶🔶🔶🔶🔶🔶🔶🔶🔶🔶🔶🔶🔶🔶🔶🔶🔶🔶🔶🔶🔶🔶🔶
# Лаборатория: Принудительная привязка профиля OAuth
...
я смог найти несколько мест где можно было бы поискать уязвимость
...
(пишу после того, как прорешал все лабы, конечно, я не знал этого, это же простая csrf ))))))
...
```
и тогда система привяжет автоматически его аккаут 

(к моему коду - к моей соц сети) -  JlT6TD3L4XCXqJv9GKSRmAOT1dYlUmZf9ixbCj62rgx
...
(пишу после того, как прорешал уже почти все лабы, и понимаю, что не так уже и сложно тут все ))))

-----
# *
Эта лаборатория с подвохом - она **имитирует реальность, где уже есть способ доставить вредоносную страницу жертве** (администратору).  задача - не придумать _как_ её доставить, а **понять, что именно нужно доставить и почему это сработает**.
(эксплойт сервер)


 единственная цель в этой лаборатории это-
...
жертве кидаем сслыку на мой сервер

а на моем эксплойт сервере лежит скрипт
```html
...
который привязывает его профиль к моему!

это сработало, потому что не было привязки токена к кукам создателя токена
